...
profileRoute.post("/profile/edit", userAuth, async (req, res) => {
  try {
    ...
    Object.keys(dataToBeUpdate).forEach(
      (key) => (LoggedInUser[key] = dataToBeUpdate[key]),
    );
...
profileRoute.patch("/profile/password", userAuth, async (req, res) => {
  try {
    ...
    res
      .status(400)
      .json({ message: "Error In Changing Password", error: err.message });
  }
});
